...
EN = {
  ...
  "meta": ["AGENTS.md", "3,125 bytes", "7 steps", "5 agent paths"],
  ...
  "steps": [
    ("Explore", "Read the repository instructions, domain model, and real data shapes, then the relevant tests, contracts, and closest matching code. Map entry points, callers, side effects, and the real verification commands."),
    ...
    ("Report", "Simplified Plain Language: one idea per sentence, every term defined. Use the project's language from <code>CONTEXT.md</code>, the glossary, and ADRs; flag any term that differs from the code. Sections in order: context, what changed, what stayed untouched, status. Number behavior changes; do not group them by file. State what I must do next. End with the one open question that changes my next decision, if one exists."),
  ],
  ...
  "expects": [
    ...
    ("Skills it names", "<b><code>brainstorming</code>, <code>writing-plans</code></b> — drop the mention if you do not have them."),
...
KO = dict(EN, **{
  ...
  "meta": ["AGENTS.md", "3,125 bytes", "7단계", "에이전트 경로 5개"],
  "copy": "계약 전문 복사", "repo": "저장소", "raw": "원문 파일",
  "s1": "서두",
  "s1lead": "루프가 시작되기 전, 세션 내내 걸려 있는 세 가지 규칙이다. 아래는 한국어 설명이고, 복사 버튼으로 받는 원문은 영어 파일 그대로다.",
  "clauses": [
    ("태도", "테스트는 도메인 모델을 검증할 뿐, 정의하지는 않는다. 데이터 손실과 공개 API, 보안, 마이그레이션에 관한 것만 묻는다. 나머지는 가정을 밝히고 그대로 진행한다. 작업은 좁게 잘라 새 컨텍스트의 서브에이전트에 넘긴다. 목표와 후보 경로, 제약, 기대 산출물을 적어 주고, 결과가 크면 파일로 받는다. 고칠 파일과 심볼을 이미 알거나 사소한 수정이면 직접 한다. worktree에서 한 작업은 끝나는 대로 병합하되, 대상 브랜치가 분명하지 않으면 물어본다."),
  ...
  "steps": [
    ("Explore <span class=\"gl\">탐색</span>", "저장소 지침과 도메인 모델, 실제 데이터 형태를 먼저 읽는다. 그다음 관련 테스트와 계약, 가장 비슷한 코드를 읽는다. 진입점과 호출자, 부수 효과, 그리고 실제로 돌아가는 검증 명령까지 짚어 둔다."),
    ...
    ("Plan <span class=\"gl\">계획</span>", '<code>task type · goal · files · contracts · verification · assumptions</code>를 적는다. 목표는 검증할 수 있는 형태로 쓴다. "버그를 고친다"가 아니라 "실패하는 재현 테스트를 먼저 쓰고, 그 테스트를 통과시킨다"처럼. 무엇이 바뀌면 안 되는지 적는다. 계획은 <code>writing-plans</code>로 남긴다. 계획 승인이 사람이 개입하는 마지막 지점이다. 그 뒤로는 검토와 실행, 증거 수집, 보고를 알아서 끝낸다.'),
    ...
    ("Report <span class=\"gl\">보고</span>", "쉬운 표준 문장(Simplified Plain Language)으로 쓴다. 한 문장에 한 가지 생각만 담고, 쓰는 용어는 모두 정의한다. <code>CONTEXT.md</code>와 용어집, ADR에 있는 프로젝트 언어를 쓰고, 코드와 어긋나는 용어는 짚어 준다. 순서는 맥락, 바뀐 것, 손대지 않은 것, 상태. 동작 변화에는 번호를 붙이고 파일별로 묶지 않는다. 내가 다음에 해야 할 일을 밝힌다. 내 다음 결정을 바꿀 열린 질문이 하나 있다면 그것으로 끝낸다."),
  ],
  ...
  "expects": [
    ...
    ("본문이 부르는 스킬", "<b><code>brainstorming</code>, <code>writing-plans</code></b> — 없다면 해당 문장을 지우면 된다."),
