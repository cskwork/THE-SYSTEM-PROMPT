...
KO = dict(EN, **{
  ...
  "clauses": [
    ("태도", "계획이 승인된 뒤에는 데이터 손실과 공개 API, 보안, 마이그레이션에 관한 것만 묻는다. 나머지는 가정을 밝히고 그대로 진행한다. 작업은 좁게 잘라 새 컨텍스트의 서브에이전트에 넘긴다. 목표와 후보 경로, 제약, 기대 산출물을 적어 주고, 결과가 크면 파일로 받는다. 고칠 파일과 심볼을 이미 알거나 사소한 수정이면 직접 한다. worktree에서 한 작업은 끝나는 대로 병합하되, 대상 브랜치가 분명하지 않으면 물어본다."),
