...
KO = dict(EN, **{
  ...
  "steps": [
    ...
    ("Intent <span class=\"gl\">의도</span>", "내가 원하는 것을 한 문장으로 다시 말한다. 누가 그 문제를 겪는지, 무엇이 관찰 가능한 완료 조건인지까지 담는다. 내가 해결책부터 꺼냈다면 그 해결책이 푸는 문제가 무엇인지 되묻는다. 한 번에 하나씩, 다섯 개를 넘기지 않고 단단히 캐묻는다. 주장마다 <code>verified: 방법</code> 또는 <code>assumed: 이유</code>를 붙인다. 내가 한 말도 예외가 아니다. 다섯 번을 물어도 흐릿하면 결정된 것과 열린 것을 나눠 적고, 짐작하는 대신 맨 위 열린 항목부터 푼다."),
