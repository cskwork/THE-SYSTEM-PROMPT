...
KO = dict(EN, **{
  ...
  "title": "THE-SYSTEM-PROMPT — 모든 코딩 에이전트에 물리는 운영 계약",
  "desc": "파일 하나, AGENTS.md. 태도와 증거 규칙, 그리고 orient에서 report까지 이어지는 7단계 루프. Claude Code·Codex CLI·Gemini CLI·OpenCode·pi에 심링크로 걸어 쓴다.",
  "lede": '모든 코딩 에이전트에 물리는 운영 계약. <b>파일 하나, <code>AGENTS.md</code></b>에 태도와 증거 규칙, orient에서 report까지의 7단계 루프가 들어 있다.',
  "meta": ["AGENTS.md", "2,861 bytes", "7단계", "에이전트 경로 5개"],
  "copy": "계약 전문 복사", "repo": "저장소", "raw": "원문 파일",
  "s1": "서두",
  "s1lead": "루프가 시작되기 전, 세션 전체에 걸리는 네 가지 규칙. 아래는 한국어 설명이고, 복사 버튼이 주는 원문은 영어 파일 그대로다.",
  "clauses": [
    ("태도", "테스트는 도메인 모델을 검증할 뿐, 정의하지 않는다. 데이터 손실, 공개 API, 보안, 마이그레이션에 대해서만 묻는다. 그 밖에는 가정을 밝히고 진행한다. worktree 작업은 끝나면 병합하고, 대상 브랜치가 불분명하면 물어본다."),
    ("주장보다 증거", "저장소 문서와 주석, 그리고 내가 하는 말도 낡는다. 실행 중인 코드, 실제 데이터, 또는 정본으로 확인한다. 증거가 내 말과 어긋나면 근거를 들어 반박한다. 그래도 불확실하면 묻는다."),
    ("도메인 규칙", "언제나 <code>~/.agents/rules/rules.md</code>를 읽는다."),
    ("글쓰기", "보고서, 커밋 메시지, 주석, 문서까지 모든 산문에 <code>unslop</code> 스킬을 적용한다."),
  ],
  "s2": "7단계 루프",
  "s2lead": "orient에서 report까지. 에이전트가 사람을 기다리는 지점은 4단계 하나뿐이다.",
  "steps": [
    ("Orient <span class=\"gl\">파악</span>", "저장소 지침, 도메인 모델, 실제 데이터 형태를 먼저 읽고, 이어서 관련 테스트와 계약, 가장 비슷한 코드를 읽는다. 진입점, 호출자, 부수 효과, 그리고 실제로 돌아가는 검증 명령을 지도로 만든다."),
    ("Options <span class=\"gl\">선택지</span>", "계획이나 코드를 내기 전에, 전략이 서로 다른 세 가지 접근을 한 줄씩 제시한다. 각 줄은 접근, 핵심 트레이드오프, 비용 또는 위험. 순위를 매기고 1순위를 고른 이유를 한 가지 대고, 거기서 멈춰 선택을 요청한다. 한 가지 접근만 명백히 합리적일 때만 건너뛴다."),
    ("Delegate <span class=\"gl\">위임</span>", "좁은 작업을 새 컨텍스트의 서브에이전트에 보낸다. 각 작업은 목표, 후보 경로, 제약, 기대 산출물을 명시한다. 큰 결과는 파일로 돌려받아 독립적으로 검증한다. 파일과 심볼을 이미 알거나 사소한 한 번의 수정이면 위임하지 않는다."),
    ("Plan <span class=\"gl\">계획</span>", '<code>task type · goal · files · contracts · verification · assumptions</code>를 적는다. 목표는 검증 가능한 형태로 쓴다. "버그를 고친다"가 아니라 "실패하는 재현 테스트를 쓰고, 그것을 통과시킨다". 의도가 불분명하면 <code>brainstorming</code>으로 한 번에 한 가지씩 묻되 확신이 95%에 이를 때까지 한다. 계획은 <code>writing-plans</code>로 기록한다. 계획 승인이 마지막 사람 게이트다. 그 뒤로는 검토, 실행, 검증, 보고를 알아서 한다.'),
    ("Execute <span class=\"gl\">실행</span>", "계획을 따른다. 현실이 다르면 계획 게이트를 다시 통과한다. 추상화는 전체 인지 부하를 줄이거나 실제 변형을 지탱할 때만 넣는다. 내 변경으로 쓰이지 않게 된 import·변수·함수는 지우고, 원래부터 죽어 있던 코드는 그대로 두되 언급한다."),
    ("Verify <span class=\"gl\">검증</span>", "회귀, 단위, 통합, 타입, 린트, 빌드, 재현 검사 중 해당되는 것을 돌린다. 명령과 실제 출력을 보여주고, 통과 / 기존 실패 / 회귀 / 건너뜀 / 환경 제약으로 나눠 정리한다."),
    ("Report <span class=\"gl\">보고</span>", "Simplified Technical English로 쓴다. 한 문장에 한 가지 생각, 모든 용어는 정의한다. <code>CONTEXT.md</code>와 용어집, ADR의 프로젝트 언어를 쓰고, 코드와 다른 용어는 표시한다. 순서는 맥락, 바뀐 것, 손대지 않은 것, 상태. 동작 변화에는 번호를 붙이고 파일별로 묶지 않는다. 내가 다음에 해야 할 일을 밝힌다. 내 다음 결정을 바꿀 열린 질문이 하나 있으면 그것으로 끝낸다."),
  ],
  "gate": "마지막 사람 게이트", "unattended": "여기서부터 무인 진행",
  "s3": "설치", "s3lead": "정본 한 벌을 두고, 그 파일을 각 에이전트의 설정 디렉터리에 링크한다.",
  "gemini": "# Gemini CLI는 GEMINI.md를 읽는다",
  "copyinstall": "설치 블록 복사",
  "caption": "에이전트별 로드 경로", "th": ("에이전트", "읽는 경로"),
  "caveatlabel": "실행하기 전에",
  "caveat": "대상이 심링크가 아니라 일반 파일이면 먼저 타임스탬프 백업으로 옮긴다. 설치하지 않은 에이전트의 디렉터리는 건너뛴다. Windows에서 심링크는 개발자 모드나 관리자 터미널이 필요하다. 그럴 땐 복사하되, 사본은 원본과 어긋난다는 점을 감안한다.",
  "s4": "무엇을 뺐는가",
  "s4lead": "이 파일은 하네스 프롬프트를 대체하지 않고 그 위에 얹힌다. 그래서 기본 동작을 실제로 바꾸는 규칙만 담았다. 코드 전에 순위 매긴 세 가지 안 내놓기, 문서가 아니라 실행 코드로 주장 검증하기, 검증 출력을 통과·기존 실패·회귀·건너뜀으로 분류하기, 정해진 순서로 보고하기.",
  "expects": [
    ("매 세션 읽는다", "<b><code>~/.agents/rules/rules.md</code></b> — 당신의 도메인 규칙."),
    ("이름을 부르는 스킬", "<b><code>unslop</code>, <code>brainstorming</code>, <code>writing-plans</code></b> — 없으면 해당 문장을 지운다."),
    ("있으면 읽는다", "저장소의 <b><code>CONTEXT.md</code>, 용어집, ADR</b>."),
  ],
  "footer": "cskwork/pi-setup과 cskwork/pi-setup-public이 같은 파일을 싣는다. pi 설치기가 ~/.pi/agent/AGENTS.md를 그쪽 사본에 링크하기 때문이다. 정본은 이 저장소이고, 수정은 여기서 먼저 한다.",
